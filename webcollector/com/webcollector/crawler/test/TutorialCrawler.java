...
import com.webcollector.crawler.test.TutorialCrawler;
...
/**
 * WebCollector 2.x版本的tutorial 2.x版本特性：
 * 
 ...
 */
/*
 * 2.x版本中，爬虫的遍历由用户自定义(本质还是广度遍历，但是每个页面生成的URL，也就是遍历树中每个节点的孩子节点，是由用户自定义的)。
 * 
 * 1.x版本中，默认将每个页面中，所有满足正则约束的链接，都当作待爬取URL，通过这种方法可以完成在一定范围内(例如整站)的爬取(根据正则约束)。
 * 
 * 所以在2.x版本中，我们只要抽取页面中满足正则的URL，作为Links返回，就可以完成1.x版本中BreadthCrawler的功能。
 */
public class TutorialCrawler extends DeepCrawler {
	...
	public Links visitAndGetNextLinks(Page page) {
		...
		if (jdbcTemplate != null) {
			int updates = jdbcTemplate.update(
					"insert into tb_content (title,url,html) value(?,?,?)",
	...
	public static void main(String[] args) throws Exception {
		// 构造函数中的string,是爬虫的crawlPath，爬虫的爬取信息都存在crawlPath文件夹中,不同的爬虫请使用不同的crawlPath
		TutorialCrawler crawler = new TutorialCrawler("tc");
		...
		/*
		 * 可用代理可以到 http://www.brieftools.info/proxy/ 获取 添加代理的方式:
		 * 1)ip和端口proxys.add("123.123.123.123",8080);
		 * 
		 * 2)文件 proxys.addAllFromFile(new File("xxx.txt")); 文件内容类似:
		 * 123.123.123.123:90 234.234.324.234:8080 一个代理占一行
		 */
